...
try:
    ...
    def _gc_client():
        # Requiere st.secrets["gcp_service_account"] cargado en Streamlit Cloud
    ...
    # Si falta gspread/oauth2client o secrets, el módulo de vigilancia avisará.
    GS_READY = False
...
def modulo_vigilancia():
    ...
    with col2:
        ...
        if mostrar_curva_epidemica:
            ...
            if not GS_READY:
                st.warning("Conecta Google Sheets (gspread + oauth2client y secrets) para mostrar la curva.")
            else:
                try:
        ...
        if mostrar_censo:
            ...
            if not GS_READY:
                st.warning("Conecta Google Sheets (gspread + oauth2client y secrets) para mostrar el censo en vivo.")
            else:
                try:
