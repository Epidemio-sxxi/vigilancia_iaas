...
try:
    ...
    _gs_err = str(e1)
    # Intento 2: compatibilidad con oauth2client
    try:
...
def modulo_vigilancia():
    ...
    with st.expander("Estado de conexión a Google Sheets", expanded=False):
        if GS_READY:
            st.success("Conexión preparada (credenciales cargadas). Sheet ID: " + SHEET_ID)
        else:
            st.error("Google Sheets no listo. " + (f"Detalle: {_gs_err}" if _gs_err else ""))
            st.info("Sube el JSON del Service Account a st.secrets como 'gcp_service_account' y comparte la hoja con ese correo.")
                ...
                try:
                    ...
                    if df_vivo.empty:
                        ...
                        if "servicio" in df_censo.columns and not df_censo.empty:
                            top_srv = (
                                df_censo.groupby("servicio")["servicio"]
                                .count()
